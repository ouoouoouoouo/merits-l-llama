"""Stage II trainer: Bi-GRU + self-attention over precomputed Llama features.

Ported from merits-l-text/src/train_stage2.py — model is encoder-agnostic,
only cfg.model.input_dim differs (RoBERTa 1024 -> Llama 4096).
...
def train(cfg: AttrDict) -> None:
    ...
    runlog.info("=" * 70)
    runlog.info(f"Features:     {cfg.dataset.features_path}")
    runlog.info(f"Model:        Bi-GRU input_dim={cfg.model.input_dim}  "
                f"gru_hidden={cfg.model.gru_hidden}  layers={cfg.model.get('gru_layers', 2)}  "
